  ...
  //  [Authorize(Helper.Admin)]
    public class CategoriesController : Controller
    {
        ...
        public CategoriesController(IServicesRepository<Category> servicesCategory,
               IServicesRepositoryLog<LogCategory> servicesCategoryLog, UserManager<ApplicationUser> userManager)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {// Update
                    if (_servicesCategory.Save(model.NewCategory) && _servicesCategoryLog.Update(model.NewCategory.Id, Guid.Parse(userId)))
                        SessionMsg(Helper.Success, Resource.ResourceWeb.lbSave,Resource.ResourceWeb.lbMsgUpdateCategory);
                    else
